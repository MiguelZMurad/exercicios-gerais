...
tBanco *CriaBanco() {
    ...
    b->a = (tAgencia**) malloc (sizeof(tAgencia*));
    b->qtdAgencia = 0;
...
void DestroiBanco(tBanco *banco) {
    ...
    for (i = 0; i < banco->qtdAgencia; i++) {
        DestroiAgencia(banco->a[i]);
    }
    free(banco->a);
    free(banco);
}
...
void AdicionaAgencia(tBanco *banco, tAgencia *agencia) {
    ...
    banco->a = realloc (banco->a , banco->qtdAgencia * sizeof(tAgencia*));
    banco->a[banco->qtdAgencia-1] = agencia;
}
...
void InsereContaBanco(tBanco *banco, int numAgencia, tConta *cliente) {
    ...
    for (i = 0; i < banco->qtdAgencia; i++) {
        if (ComparaAgencia(numAgencia, banco->a[i])) {
            AdicionaConta(banco->a[i], cliente);
        }
    }
}
